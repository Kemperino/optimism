...
func (s ConductorSet) VerifyUnsafeChainAdvancesAndConverges(delta uint64) {
	...
	ref := s[0].Sequencer()
	checks := make([]CheckFunc, 0, 2*len(s)-1)
	for _, con := range s {
		checks = append(checks, con.Sequencer().AdvancedFn(safety.LocalUnsafe, delta, conductorSettleAttempts))
	}
	for _, con := range s[1:] {
		checks = append(checks, con.Sequencer().InSyncFn(ref, safety.LocalUnsafe, conductorSettleAttempts))
	}
	CheckAll(c.t, checks...)
